...
export default function TraderProfile() {
  ...
  if (loading) return <div className="max-w-7xl mx-auto px-4 py-20 text-[#A0A8C0]">Loading profile…</div>;
  if (!trader) return <div className="max-w-7xl mx-auto px-4 py-20 text-[#A0A8C0]">Trader not found.</div>;

  const tr = trader;
  const username = tr.discord_username || tr.referral_code || "Trader";

  const stats = [
    { label: "REX Balance", value: (tr.rex_balance || 0).toLocaleString(), icon: Zap, color: "#D4AF37" },
    { label: "Current Streak", value: tr.checkin_streak || 0, icon: Flame, color: "#D4AF37" },
    { label: "Best Streak", value: tr.best_streak || 0, icon: Trophy, color: "#00C853" },
    { label: "Total Check-ins", value: tr.total_checkins || 0, icon: CheckCircle2, color: "#00C853" },
  ];

  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      <Link to="/traders" className="inline-flex items-center gap-2 text-[#A0A8C0] hover:text-white transition-colors mb-8">
      ...
      <div className="card mb-10 fade-in">
        <div className="flex flex-col md:flex-row md:items-center gap-6">
          <div className="w-20 h-20 rounded-2xl flex items-center justify-center text-2xl font-bold text-white shrink-0" style={{ backgroundImage: "linear-gradient(135deg, #D4AF37, #00C853)" }}>
            {username[0].toUpperCase()}
          </div>
          <div className="flex-1">
            <div className="flex items-center gap-2 mb-1">
              <h1 className="text-2xl font-bold text-white">{username}</h1>
              {tr.verified && <BadgeCheck className="w-5 h-5 text-[#00C853]" />}
              {tr.banned && <span className="badge" style={{ backgroundColor: "rgba(255,59,59,0.15)", color: "#FF3B3B" }}>Banned</span>}
            </div>
            <div className="flex flex-wrap items-center gap-4 text-sm text-[#A0A8C0]">
      ...
      <div className="space-y-3">
        {activity.length === 0 ? (
          <div className="card text-center text-[#A0A8C0] py-12">No activity yet.</div>
        ) : (
          activity.map((a) => {
            ...
            return (
              <div key={a.id} className="card flex items-center gap-4" style={{ padding: "16px 20px" }}>
                <div className="w-10 h-10 rounded-xl flex items-center justify-center shrink-0" style={{ backgroundColor: isCheckin ? "rgba(212,175,55,0.15)" : positive ? "rgba(0,200,83,0.15)" : "rgba(255,59,59,0.15)" }}>
                  {isCheckin ? <Flame className="w-5 h-5 text-[#D4AF37]" /> : positive ? <Gift className="w-5 h-5 text-[#00C853]" /> : <Zap className="w-5 h-5 text-[#FF3B3B]" />}
                </div>
                <div className="flex-1 min-w-0">
                  <div className="text-white font-semibold text-sm">
                    {isCheckin ? `Daily check-in — streak ${a.new_streak}` : a.description || a.type?.replace(/_/g, " ")}
                  </div>
                  <div className="text-xs text-[#A0A8C0]">
                ...
                <div className="text-right">
                  <span className="font-bold text-sm" style={{ color: isCheckin || positive ? "#00C853" : "#FF3B3B" }}>
