    ...
    return (
      <div className="max-w-2xl mx-auto px-4 py-24 text-center">
        <div className="card">
          ...
          <p className="text-[#A0A8C0]">Connect your Discord to create your FORTREX trader profile and view transactions.</p>
        </div>
      </div>
    );
  }
    ...
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-10">
        <div className="card text-center fade-in">
          ...
          <div className="text-3xl font-bold text-white">{earned.toLocaleString()} <span className="text-[#00C853] text-base">REX</span></div>
        </div>
        <div className="card text-center fade-in">
          <div className="section-label mb-3">Total Spent</div>
          <div className="text-3xl font-bold text-white">{spent.toLocaleString()} <span className="text-[#FF3B3B] text-base">REX</span></div>
        </div>
        <div className="card text-center fade-in">
          <div className="section-label mb-3">Current Balance</div>
          <div className="text-3xl font-bold text-white">{balance.toLocaleString()} <span className="text-[#D4AF37] text-base">REX</span></div>
        </div>
      </div>

      <div className="space-y-3">
        {txns.length === 0 ? (
          <div className="card text-center text-[#A0A8C0] py-12">No transactions yet.</div>
        ) : (
          txns.map((t) => {
            ...
            return (
              <div key={t.id} className="card flex items-center gap-4" style={{ padding: "16px 20px" }}>
                <div className="w-10 h-10 rounded-xl flex items-center justify-center shrink-0" style={{ backgroundColor: positive ? "rgba(0,200,83,0.15)" : "rgba(255,59,59,0.15)" }}>
                  {positive ? <ArrowDownLeft className="w-5 h-5 text-[#00C853]" /> : <ArrowUpRight className="w-5 h-5 text-[#FF3B3B]" />}
                </div>
                <div className="flex-1 min-w-0">
                  ...
                  {t.description && <div className="text-xs text-[#A0A8C0] mt-1">{t.description}</div>}
                </div>
                <div className="text-right">
                  <span className="font-bold text-sm" style={{ color: positive ? "#00C853" : "#FF3B3B" }}>
