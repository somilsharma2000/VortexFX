...
export default function Referrals() {
    ...
    return (
      <div className="max-w-2xl mx-auto px-4 py-24 text-center">
        <div className="card">
          ...
          <p className="text-[#9ca3af]">Connect your Discord to create your Koda trader profile and start referring.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      ...
      <div className="card mb-8 fade-in text-center" style={{ backgroundImage: "linear-gradient(135deg, rgba(139,92,246,0.1), rgba(59,130,246,0.04))" }}>
        <div className="inline-flex items-center gap-2 section-label mb-4 justify-center"><Gift className="w-4 h-4 text-[#7c3aed]" /> Your Referral Code</div>
        ...
        <div className="flex flex-col sm:flex-row gap-3 max-w-xl mx-auto">
          <div className="input-field flex-1 flex items-center" style={{ padding: "12px 16px" }}>
            <span className="text-sm text-[#9ca3af] truncate">{link}</span>
          </div>
      ...
      <div className="card overflow-hidden" style={{ padding: 0 }}>
        ...
        {referrals.length === 0 ? (
          <div className="px-6 py-8 text-[#9ca3af]">No referrals yet. Share your code to earn REX.</div>
        ) : (
          referrals.map((r) => (
            <div key={r.id} className="data-row grid grid-cols-12 items-center px-6 py-4 border-b border-[#202028] last:border-0">
              ...
              <div className="col-span-3 text-sm text-[#9ca3af]">{r.referral_date ? new Date(r.referral_date).toLocaleDateString(undefined, { month: "short", day: "numeric", year: "numeric" }) : "—"}</div>
