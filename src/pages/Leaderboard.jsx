...
export default function Leaderboard() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      <SectionHeader label="Rankings" title="Platform Leaderboard" subtitle="The top FORTREX traders ranked by total REX earned across all tournaments and rewards." />
      ...
      <div className="card overflow-hidden" style={{ padding: 0 }}>
        ...
        {loading ? (
          <div className="px-6 py-8 text-[#A0A8C0]">Loading leaderboard…</div>
        ) : filtered.length === 0 ? (
          <div className="px-6 py-8 text-[#A0A8C0]">No traders found.</div>
        ) : (
          filtered.map((tr, i) => (
            <Link key={tr.id} to={`/traders/${tr.id}`} className="data-row grid grid-cols-12 items-center px-6 py-4 border-b border-[#D4AF37]/15 last:border-0">
              <div className="col-span-1 flex items-center">
                {i < 3 ? (
                  <Crown className="w-5 h-5" style={{ color: i === 0 ? "#D4AF37" : i === 1 ? "#D4AF37" : "#00C853" }} />
                ) : (
                  <span className="text-[#A0A8C0] font-semibold">{i + 1}</span>
                )}
              </div>
              ...
              <div className="col-span-2 text-right text-[#A0A8C0] hidden md:block">{tr.total_checkins || 0}</div>
