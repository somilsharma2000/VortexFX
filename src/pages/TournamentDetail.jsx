...
export default function TournamentDetail() {
  ...
  if (loading) return <div className="max-w-7xl mx-auto px-4 py-20 text-[#A0A8C0]">Loading tournament…</div>;
  if (!tournament) return <div className="max-w-7xl mx-auto px-4 py-20 text-[#A0A8C0]">Tournament not found.</div>;
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      <Link to="/tournaments" className="inline-flex items-center gap-2 text-[#A0A8C0] hover:text-white transition-colors mb-8">
        <ArrowLeft className="w-4 h-4" /> Back to tournaments
      </Link>

      {/* Hero card */}
      <div className="card mb-10 fade-in" style={{ backgroundImage: "linear-gradient(135deg, rgba(212,175,55,0.08), rgba(0,200,83,0.04))" }}>
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
          ...
          <div className="text-right">
            ...
            <div className="text-4xl font-bold text-white">{(t.prize_pool_rex || 0).toLocaleString()} <span className="text-[#D4AF37] text-lg">REX</span></div>
          </div>
        </div>
      </div>
      ...
      <div className="card overflow-hidden" style={{ padding: 0 }}>
        ...
        {leaders.length === 0 ? (
          <div className="px-6 py-8 text-[#A0A8C0]">No participants yet.</div>
        ) : (
          leaders.map((tr, i) => (
            <Link key={tr.id} to={`/traders/${tr.id}`} className="data-row grid grid-cols-12 items-center px-6 py-4 border-b border-[#D4AF37]/15 last:border-0">
              <div className="col-span-1 flex items-center">
                {i === 0 ? <Crown className="w-5 h-5 text-[#D4AF37]" /> : <span className="text-[#A0A8C0] font-semibold">{i + 1}</span>}
              </div>
              ...
              <div className="col-span-2 text-right">
                <span className="inline-flex items-center gap-1 text-white font-medium"><Flame className="w-4 h-4 text-[#D4AF37]" />{tr.checkin_streak || 0}</span>
              </div>
