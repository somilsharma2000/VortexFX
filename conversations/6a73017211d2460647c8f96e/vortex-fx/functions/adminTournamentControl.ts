...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const actions = {
      ...
      pause: { status: 'upcoming', is_active: false },
      reveal: { status: 'revealing' },
      complete: { status: 'completed', is_active: false },
      cancel: { status: 'cancelled', is_active: false }
    };
    ...
    return Response.json({ success: false, error: 'Action failed.' });
  }
});