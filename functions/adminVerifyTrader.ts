...
Deno.serve(async (req) => {
  ...
  try {
    ...
    try {
      ...
      if (trader && trader.email) {
        const wl = await base44.asServiceRole.entities.WaitlistEntry.filter({ email: trader.email });
  ...
  } catch (err) { return Response.json({ success: false, error: 'Verification failed.' }); }
});